...
export function PartyForm({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      closeDialog();
      showToast({ message: method === "PATCH" ? "Đã cập nhật đối tác thành công." : "Đã tạo đối tác thành công." });
    ...
    >
      <FormGrid>
        <Controller
          ...
          name="type"
          render={({ field }) => (
            <AppSelect label="Loại *" {...field} error={Boolean(errors.type)} helperText={errors.type?.message} value={field.value ?? "CUSTOMER"}>
        ...
        />
        <AppInput label="Mã" placeholder="KH-001" {...register("code")} error={Boolean(errors.code)} helperText={errors.code?.message} />
        <AppInput label="Tên *" placeholder="Công ty ABC" {...register("name")} error={Boolean(errors.name)} helperText={errors.name?.message} />
        <AppInput label="Điện thoại" {...register("phone")} error={Boolean(errors.phone)} helperText={errors.phone?.message} />
        <AppInput label="Email" type="email" {...register("email")} error={Boolean(errors.email)} helperText={errors.email?.message} />
        <AppInput label="Mã số thuế" {...register("taxCode")} error={Boolean(errors.taxCode)} helperText={errors.taxCode?.message} />
        <AppInput label="Hạn mức công nợ" placeholder="500000000" type="number" {...register("creditLimit")} error={Boolean(errors.creditLimit)} helperText={errors.creditLimit?.message} />
        <Controller
          control={control}
          name="assignedToId"
          render={({ field }) => (
            <AppSelect label="Sale phụ trách" {...field} error={Boolean(errors.assignedToId)} helperText={errors.assignedToId?.message} value={field.value ?? ""}>
              <MenuItem value="">Chưa gán</MenuItem>
              {initialValues?.assignedToId && !users.some((user) => user.id === initialValues.assignedToId) ? (
      ...
      <AppInput label="Địa chỉ" {...register("address")} error={Boolean(errors.address)} helperText={errors.address?.message} />
      ...
      <FormActions><Button loading={mutation.isPending} type="submit">{submitLabel}</Button></FormActions>
    </Box>
  );
}
