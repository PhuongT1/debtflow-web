...
export function PartyDetailClient({ partyId }: { partyId: string }) {
  ...
  const metrics = useMemo(() => {
    ...
    const overdue = active
      .filter((debt) => new Date(debt.dueDate).getTime() < Date.now())
      .reduce((sum, debt) => sum + remainingAmount(debt), 0);
  ...
  const columns: Array<DataTableColumn<Debt>> = [
      ...
      render: (debt) => (
        <Box>
          <Typography
            component="a"
            href={`/debts/${debt.id}`}
            sx={{ color: 'primary.main', fontSize: 13.5, fontWeight: 750, textDecoration: 'none' }}
          >
            {debt.code}
          </Typography>
    ...
    },
    {
      key: 'document',
      header: 'Chứng từ',
      render: (debt) => debt.invoiceNo ?? debt.orderNo ?? debt.contractNo ?? '-',
    },
    {
      key: 'remaining',
      header: 'Còn lại',
      align: 'right',
      render: (debt) => formatMoney(remainingAmount(debt)),
    },
    {
      key: 'dueDate',
      header: 'Hạn',
      render: (debt) => new Date(debt.dueDate).toLocaleDateString('vi-VN'),
    },
    {
      key: 'status',
      header: 'Trạng thái',
      render: (debt) => (
        <Badge
          tone={debt.status === 'PAID' ? 'green' : debt.status === 'CANCELLED' ? 'slate' : 'amber'}
        >
          {debt.status}
        </Badge>
      ),
    ...
    <Page>
      ...
      <MetricGrid columns={5}>
        ...
        <StatCard
          ...
          label="Còn phải trả"
          tone="amber"
          value={formatMoney(metrics.payable)}
        />
        <StatCard icon="calendar" label="Quá hạn" tone="red" value={formatMoney(metrics.overdue)} />
        <StatCard
          icon="aging"
          label="Hạn mức"
          value={party.creditLimit ? formatMoney(party.creditLimit) : 'Chưa đặt'}
        />
