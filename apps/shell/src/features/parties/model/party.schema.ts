import { z } from 'zod';
import { PartyType } from '@/lib/domain/enums';
...
export const partyQuerySchema = z.object({
  ...
  balanceType: optionalQueryValue(z.enum(['RECEIVABLE', 'PAYABLE'])),
  sortBy: optionalQueryValue(z.enum(['name', 'outstandingDocuments', 'receivable', 'payable'])),
  sortOrder: optionalQueryValue(z.enum(['asc', 'desc'])),
