import { DebtStatus } from "@/lib/domain";
import { apiGet, toDate, toMoney, type Money } from "@/lib/server-api";
import { debtQuerySchema } from "./schemas";
...
type RawDebt = Omit<Debt, "originalAmount" | "paidAmount" | "issueDate" | "dueDate" | "nextFollowUpAt" | "payments"> & {
...
export async function listDebts(input: unknown) {
  ...
  const result = await apiGet<Paginated<RawDebt>>("debts", { query });
...
export async function getDebt(id: string) {
  return normalizeDebt(await apiGet<RawDebt>(`debts/${id}`));
}

export async function getAgingReport(input: Record<string, unknown> = {}) {
