...
import { partyQuerySchema } from "./party-query-schema";
...
type RawPartyDebt = Omit<PartyDebt, "originalAmount" | "paidAmount" | "dueDate" | "nextFollowUpAt"> & {
...
export async function listParties(input: unknown) {
  ...
  const result = await apiGet<Paginated<RawParty>>("parties", { query });
