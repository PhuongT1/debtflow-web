...
function resolveEnabledOrigin(application: MfeApplication, environment: NodeJS.ProcessEnv) {
  const enabled = environment[application.enabledEnv] ?? 'false';
  if (enabled !== 'true' && enabled !== 'false') {
    throw new Error(`${application.enabledEnv} must be either "true" or "false".`);
  }
  if (enabled === 'false') return null;

  const rawOrigin = environment[application.originEnv];
  if (!rawOrigin) {
    throw new Error(`${application.originEnv} is required when ${application.enabledEnv}=true.`);
  }
...
export function loadMfeRewrites(
  ...
  return loadMfeManifest(manifestPath).applications.flatMap((application) => {
    ...
    const { integration, layout, publicPath, proxyPath, remoteBasePath, entryPath } =
