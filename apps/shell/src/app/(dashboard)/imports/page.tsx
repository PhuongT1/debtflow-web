...
export default function ImportsPage() {
  return (
    <Page>
      <PageHeader
        title="Import Excel"
        description="Đưa dữ liệu công nợ cũ từ Excel vào hệ thống để sale chuyển qua web nhanh hơn."
        actions={
          <HelpModal
            ...
            tips={[
              'Nên import thử 5-10 dòng trước khi import file lớn.',
              'Nếu dùng saleEmail, email đó phải tồn tại trong danh sách user.',
            ]}
          />
        }
      />
        ...
        <SectionCard title="Định dạng file" description="Danh sách cột được hỗ trợ">
          <div className="grid gap-3 text-sm text-slate-600">
            ...
            <code className="rounded-lg bg-slate-100 p-3 text-xs">
              partyName, partyCode, partyPhone, saleEmail, type, title, originalAmount, issueDate,
              dueDate, invoiceNo, orderNo, contractNo, poNo, followUpNote
            </code>
            <p>
              `type` dùng `RECEIVABLE` cho phải thu hoặc `PAYABLE` cho phải trả. Nếu không nhập
              `issueDate`, hệ thống lấy ngày hiện tại.
            </p>
          </div>
        </SectionCard>
      </PageGrid>
