...
export default async function UsersPage({
  ...
  const columns: Array<DataTableColumn<(typeof users)[number]>> = [
    ...
    { key: "email", header: "Email", render: (user) => user.email },
    {
      key: "role",
      header: "Role",
      render: (user) => <Badge tone={user.role === "ADMIN" ? "blue" : user.role === "ACCOUNTANT" ? "amber" : "slate"}>{user.role}</Badge>,
    },
    { key: "status", header: "Trạng thái", render: (user) => user.status },
    {
      key: "actions",
      header: "Thao tác",
      render: (user) => <ConfirmActionButton endpoint={`/api/users/${user.id}`} confirmMessage="Bạn chắc chắn muốn vô hiệu hóa user này?" />,
    },
  ];

  return (
    <Page fillAvailable>
      <PageHeader title="Phân quyền" description="Quản lý vai trò ADMIN, ACCOUNTANT và VIEWER." actions={
        <HelpModal
          ...
          tips={["Hiện màn hình này mới xem danh sách user. Chức năng tạo/sửa user nên làm tiếp ở phase sau."]}
        />
      } />
