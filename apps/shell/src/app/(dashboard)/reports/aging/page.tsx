...
import { getAgingReport, listDebts } from "@/features/debts/service";
...
export default async function AgingReportPage({
  ...
  const [aging, debts] = await Promise.all([getAgingReport({ type: "RECEIVABLE" }), listDebts({ ...filters, type: "RECEIVABLE" })]);
  const activeDebts = debts.items.filter((debt) => debt.status !== "PAID" && debt.status !== "CANCELLED");
  const columns: Array<DataTableColumn<(typeof debts.items)[number]>> = [
    ...
    { key: "remaining", header: "Còn lại", align: "right", render: (debt) => <span className="font-semibold">{formatMoney(debt.originalAmount.minus(debt.paidAmount))}</span> },
    { key: "dueDate", header: "Hạn", render: (debt) => debt.dueDate.toLocaleDateString("vi-VN") },
    {
      ...
      render: (debt) => {
        ...
        return <span className={lateDays > 0 ? "font-semibold text-red-600" : "text-slate-500"}>{lateDays > 0 ? `${lateDays} ngày` : "Chưa đến hạn"}</span>;
      },
    },
    {
      key: "actions",
  ...
  return (
    <Page fillAvailable>
      <PageHeader title="Báo cáo tuổi nợ" description="Nhìn nhanh khoản nào mới trễ, khoản nào cần ưu tiên xử lý ngay." actions={
        <HelpModal
      ...
      <MetricGrid columns={5}>
        {Object.values(aging).map((bucket) => (
          <StatCard icon="aging" key={bucket.label} label={bucket.label} value={formatMoney(bucket.amount)} hint={`${bucket.count} khoản`} />
        ))}
      </MetricGrid>
