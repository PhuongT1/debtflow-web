        ...
        <CardContent sx={{ p: { xs: 3, md: 4 }, display: "flex", flexDirection: "column", gap: 2.5 }}>
          <Box sx={{ display: "flex", alignItems: "center", gap: 1.5 }}>
            ...
            <Typography variant="h6" sx={{ fontWeight: 700, color: "error.main" }}>
              {isBackendDown ? "Cannot Connect to Backend Server" : "An Error Occurred While Loading Data"}
            </Typography>
          </Box>
          ...
          {isBackendDown && (
            <Box sx={{ bgcolor: "action.hover", p: 2, borderRadius: 1.5, fontSize: 13, color: "text.secondary" }}>
              ...
              <ul style={{ margin: 0, paddingLeft: 20, lineHeight: 1.6 }}>
                <li>Make sure the <strong>debtflow-api</strong> backend service is running (e.g. <code>npm run start:dev</code>).</li>
                <li>Verify your <code>API_URL</code> environment variable configuration in platform <code>.env</code>.</li>
                <li>Once the backend server is active, click <strong>Try Again</strong> below.</li>
              </ul>
            </Box>
          )}
