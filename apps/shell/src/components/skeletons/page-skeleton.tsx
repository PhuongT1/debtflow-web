    ...
    <Box sx={{ alignItems: 'center', display: 'flex', gap: 2, justifyContent: 'space-between' }}>
      ...
      {actions ? (
        <Box sx={{ display: { xs: 'none', sm: 'flex' }, gap: 1 }}>
          <AppSkeleton height={38} width={126} />
          <AppSkeleton height={38} width={104} />
        </Box>
      ) : null}
    </Box>
  );
      ...
      {Array.from({ length: count }, (_, row) => (
        <Box
          ...
          sx={{
            ...
            display: 'grid',
            gap: 2,
            gridTemplateColumns: '1fr 1.7fr 1fr 1fr 1.2fr',
            px: 2,
            py: 1.5,
          }}
        >
          {Array.from({ length: 5 }, (_, column) => (
            <AppSkeleton
              height={16}
              key={column}
              width={`${[78, 64, 86, 56][(row + column) % 4]}%`}
            />
          ))}
        </Box>
      ))}
      ...
      >
        <Box
          sx={{
            display: 'grid',
            gap: 1.5,
            gridTemplateColumns: { xs: '1fr', md: '2fr repeat(4, minmax(120px, 1fr))' },
            p: 1.5,
          }}
        >
          {Array.from({ length: 5 }, (_, index) => (
            <AppSkeleton height={40} key={index} />
          ))}
        </Box>
        <Box
          sx={{
        ...
        >
          {Array.from({ length: 5 }, (_, index) => (
            <AppSkeleton height={13} key={index} width="55%" />
          ))}
        </Box>
    ...
    <Box aria-busy="true" aria-label="Đang tải dashboard" sx={{ display: 'grid', gap: 2 }}>
      ...
      >
        {Array.from({ length: 4 }, (_, index) => (
          <AppSkeleton height={112} key={index} />
        ))}
      </Box>
      <Paper
        elevation={0}
...
export function FormPageSkeleton() {
  return (
    <Box aria-busy="true" aria-label="Đang tải biểu mẫu" sx={{ display: 'grid', gap: 2 }}>
      <PageHeadingSkeleton />
      <Box
        sx={{ display: 'grid', gap: 2, gridTemplateColumns: { xs: '1fr', xl: 'repeat(2, 1fr)' } }}
      >
