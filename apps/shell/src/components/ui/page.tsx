...
export function PageHeader({
  ...
  return (
    <Box
      component="header"
      sx={{
        alignItems: { md: compact ? 'center' : 'flex-start' },
        display: 'flex',
      ...
      <Box sx={{ minWidth: 0 }}>
        <Box sx={{ alignItems: 'center', display: 'flex', flexWrap: 'wrap', gap: 1.25 }}>
          <Typography
            component="h1"
            sx={{
              fontSize: compact ? { xs: 22, md: 24 } : { xs: 25, md: 30 },
              fontWeight: 800,
              lineHeight: 1.2,
            }}
          >
        ...
        {!compact && description ? (
          <Typography color="text.secondary" sx={{ fontSize: 14, mt: 1.25, maxWidth: 720 }}>
            {description}
          </Typography>
        ) : null}
      </Box>
  ...
  return (
    <Box
      sx={{
        ...
        gridTemplateColumns: {
          xs: '1fr',
          xl: columns === 3 ? 'repeat(3, minmax(0, 1fr))' : 'repeat(2, minmax(0, 1fr))',
        },
      }}
    >
      {children}
  ...
  return (
    <Box
      sx={{
        ...
        display: 'grid',
        gap: 3,
        gridTemplateColumns: { xs: '1fr', xl: `${asideWidth}px minmax(0, 1fr)` },
      }}
    >
      <Box>{aside}</Box>
