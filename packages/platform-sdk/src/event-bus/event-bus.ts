...
export function publishPlatformEvent<K extends keyof DebtflowEventMap>(
  ...
  window.dispatchEvent(new CustomEvent(`debtflow:v${DEBTFLOW_EVENT_VERSION}:${name}`, { detail }));
}
...
export function subscribePlatformEvent<K extends keyof DebtflowEventMap>(
  ...
  const handler = (event: Event) => listener((event as CustomEvent<DebtflowEventMap[K]>).detail);
